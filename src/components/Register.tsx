  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/auth/register", {
        ...
        body: JSON.stringify({ email, username, inviteCode, password }),
      });
    ...
    <div className="min-h-screen flex items-center justify-center bg-grid px-6">
      <div className="card-theme w-full max-w-sm p-8 rounded-2xl shadow-lg border border-white/10 backdrop-blur-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            <label className="block text-sm text-white/70 mb-2">Инвайт код</label>
            <input
              type="text"
              value={inviteCode}
          ...
          <div>
            <label className="block text-sm text-white/70 mb-2">Подтвердите пароль</label>
            <input
              type="password"
        ...
        <div className="flex gap-5 justify-center">
            ...
            <a href="/auth/log" className="text-right text-sm text-white/40 mt-5 hover:text-white/60 transition-colors cursor-pointer">
            Уже есть аккаунт?
            </a>
